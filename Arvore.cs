...
namespace Trem1
{
    class Arvore<Dado> where Dado : IComparable<Dado>, IRegistro, new()
    {
        ...
        private Queue<NoArvore<Dado>> PercursoPorNiveis(NoArvore<Dado> noAtual)
        {
            Queue<NoArvore<Dado>> saida = new Queue<NoArvore<Dado>>();
        ...
        public Queue<NoArvore<Dado>> PercursoPorNiveis()
        {
        ...
        public void DiminuirIndice(Dado procurado)
        {
            DiminuirIndice(raiz, procurado);
        }
        private void DiminuirIndice(NoArvore<Dado> noAtual, Dado procurado)
        {
            if (noAtual == null)
                return;
            else if (noAtual.Info.CompareTo(procurado) == 0)
            {
                atual = noAtual;
                atual.Indice--;
                return;
            ...
            {
                antecessor = noAtual;
                if (procurado.CompareTo(noAtual.Info) < 0)
                    DiminuirIndice(noAtual.Esq, procurado);	 // desloca apontador na 
                else                                         // próxima instância do 
                    DiminuirIndice(noAtual.Dir, procurado);	 // método
            }
        }
        ...
        private bool ExisteRec(NoArvore<Dado> local, Dado procurado)
        {
            if (local == null)
                return false;
            else if (local.Info.CompareTo(procurado) == 0)
            {
                atual = local;
                return true;
            }
        ...
        public void Incluir(int indice, Dado dadoLido)
        {
            Incluir(indice, ref raiz, dadoLido);
        }


        private void Incluir(int indice, ref NoArvore<Dado> atual, Dado dadoLido)
        {
            if (atual == null)
            {
                atual = new NoArvore<Dado>(indice, dadoLido);
            }
            ...
            else if (dadoLido.CompareTo(atual.Info) > 0)
            {
                ...
                Incluir(indice, ref apDireito, dadoLido);
                atual.Dir = apDireito;
            }
            else
            {
                NoArvore<Dado> apEsquerdo = atual.Esq;
                Incluir(indice, ref apEsquerdo, dadoLido);
                atual.Esq = apEsquerdo;
            }
        }

        public void LerArquivoDeRegistros(string[] vetor)
        {
            raiz = null;
            Dado dado = new Dado();
            ...
            void Particionar(long inicio, long fim, ref NoArvore<Dado> atual)
            {
                if (inicio <= fim)
                {
                    ...
                    dado.LerRegistro(vetor, meio); // 
                    atual = new NoArvore<Dado>((int)meio, dado);
        ...
        {
            var destino = new FileStream(nomeArquivo, FileMode.OpenOrCreate);
        ...
        public void Inserir(int indice, Dado novoDado)
        {
            bool achou = false, fim = false;
            NoArvore<Dado> novoNo = new NoArvore<Dado>(indice, novoDado);
        ...
        public void IncluirNovoRegistro(int indice, Dado novoRegistro)
        {
            if (Existe(novoRegistro)) //checa se existe registro com nomes iguais
            ...
            else
            {
                ...
                // árvore
                var novoNo = new NoArvore<Dado>(indice, novoRegistro);
        ...
        public bool ApagarNo(Dado registroARemover)
        {
            ...
            else if (atual.Dir == null)// verificará as duas outras possibilidades, exclusão de nó com 1 ou 2 filhos
            {      // neste caso, só tem o filho esquerdo
                if (atual == raiz)
                    raiz = atual.Esq;
                else
                    if (ehFilhoEsquerdo)
